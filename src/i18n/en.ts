const en = {
  ...
  mip8: {
    ...
    watch: {
      ...
      mainnet: "Mainnet",
      testnet: "Testnet",
      caption: "MIP-8 protocol gas costs compared with the latest probe",
      pattern: "Pattern",
      ...
      checkedAtBlock: "Checked at block",
      lastChecked: "Last checked",
      updated: "MIP-8 Watch updated.",
      refreshFailed: "Latest refresh failed; showing the last successful result.",
      unreachable: "The watcher could not reach the status endpoint. Try again in a moment.",
      status: {
        active: {
    ...
    collections: {
      ...
      note: "These implementations optimize locality when one call touches several related values. A single lookup is not inherently cheaper, and every net-new slot still pays state-growth cost.",
      onePage: "128 slots · one page",
      goodFor: "Good for:",
      viewSource: "View Solidity source",
      footer: "Each primitive uses a unique namespaced storage base with its lower seven bits cleared, guaranteeing that slot zero begins at a MIP-8 page boundary.",
      readDocs: "Read usage docs",
      items: {
        bitmap: {
          title: "Dense bitmap",
          metric: "32,768 bits per page",
          description: "Stores consecutive 256-bit buckets in consecutive slots, avoiding the locality loss of mapping-backed bitmap buckets.",
        ...
        vector: {
          ...
          description: "Keeps length and the first 127 values together, then fills every following page with 128 sequential values. Includes batch append and range reads.",
        ...
        ring: {
          ...
          description: "Fits head, length, and a bounded FIFO into exactly one page, so push, pop, peek, and wraparound stay page-local.",
        ...
        keyedPage: {
          ...
          description: "Derives an independently aligned page from each logical key, keeping one account, market, or position record page-local.",
        ...
        slab: {
          ...
          description: "Combines an occupancy bitmap, live count, and fixed record slots. Removed records free their index for the next insertion.",
        ...
        uint64Vector: {
          ...
          description: "Packs four 64-bit values into each word. Later pages hold 512 values while updates preserve every neighboring packed value.",
        ...
        smallBlob: {
          ...
          description: "Stores one bounded bytes value with its length and payload in a single page. Shorter rewrites clear obsolete trailing words.",
